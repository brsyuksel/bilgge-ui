...
* [`src/cljs/bilgge_ui/`](src/cljs/bilgge_ui/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/bilgge_ui/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/bilgge_ui/`](test/cljs/bilgge_ui/): test files (ClojureScript,
...
7. Clone this repo and open a terminal in the `bilgge-ui` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/bilgge_ui/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns bilgge-ui.example
  (:require [bilgge-ui.config :as config])
